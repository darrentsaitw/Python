...
while True:
	...
	if kind in discountlist:
		check = input("該種類折扣已有，請確認是否重新輸入？(y/n)",discountlist.index(kind),discountlist[discountlist(kind)-1])
		if check == "y" or check == "Y":
			discountlist.pop([kind])
			discountlist.pop([kind-1])
	else:
...
while True:
	...
	if articles in powlist :
		if "電子" in discountlist:
			discount1 = discountlist[discountlist.index("電子")-1]
		else:
			discount1 = 1
	elif articles in foodlist :
		if "食品" in discountlist:
			discount1 = discountlist[discountlist.index("食品")-1]
		else:
			discount1 = 1
	elif articles in uselist :
		if "日用品" in discountlist:
			discount1 = discountlist[discountlist.index("日用品")-1]
		else:
			discount1 = 1
	elif articles in drinklist:
		if "酒類" in discountlist:
			discount1 = discountlist[discountlist.index("酒類")-1]
		else:
			discount1 = 1
	else:
		discount1 = 1
